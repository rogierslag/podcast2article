...
describe("Spotify resolver", () => {
  it("accepts episode URLs and strips tracking parameters", () => {
    ...
    ).toBe("https://open.spotify.com/episode/abc123");
  });

  it("rejects non-Spotify and playlist links", () => {
    expect(() =>
      validateSpotifyUrl("https://example.com/episode/abc"),
    ).toThrow();
    expect(() =>
      validateSpotifyUrl("https://open.spotify.com/playlist/abc"),
    ).toThrow();
  });
  ...
  it("selects the closest public episode", () => {
    const selected = selectBestEpisode("De toekomst van openbaar vervoer", [
      {
        trackName: "Sport van vandaag",
        episodeUrl: "https://cdn.example/sport.mp3",
      },
      {
        trackName: "De toekomst van het openbaar vervoer",
        episodeUrl: "https://cdn.example/transit.mp3",
      },
    ]);
...
describe("Google Drive resolver", () => {
  ...
  it("accepts file links and canonicalizes sharing parameters", () => {
    ...
    expect(
      googleDriveFileId(`https://drive.google.com/open?id=${fileId}`),
    ).toBe(fileId);
    expect(
      validateGoogleDriveUrl(
        `https://drive.google.com/file/d/${fileId}/view?resourcekey=0-example_key`,
      ).toString(),
    ).toBe(
      `https://drive.google.com/file/d/${fileId}/view?resourcekey=0-example_key`,
    );
  ...
  it("rejects folders, Meet room links, and unrelated hosts", () => {
    expect(() =>
      validateGoogleDriveUrl(
        "https://drive.google.com/drive/folders/abc123456789",
      ),
    ).toThrow();
    expect(() =>
      validateGoogleDriveUrl("https://meet.google.com/abc-defg-hij"),
    ).toThrow();
    expect(() =>
      validateGoogleDriveUrl(`https://example.com/file/d/${fileId}/view`),
    ).toThrow();
  });
  ...
  it("rejects non-media Drive files", () => {
    expect(() =>
      parseGoogleDriveMetadata(
        `<meta property="og:title" content="Notes.pdf">`,
      ),
    ).toThrow();
  });
  ...
  it("builds the public media URL and dispatches all supported sources", () => {
    ...
    expect(googleDriveDownloadUrl(fileId, "0-example_key")).toContain(
      "resourcekey=0-example_key",
    );
    expect(
      validateSourceUrl(`https://drive.google.com/file/d/${fileId}/view`)
        .hostname,
    ).toBe("drive.google.com");
    expect(
      validateSourceUrl("https://open.spotify.com/episode/abc123").hostname,
    ).toBe("open.spotify.com");
    expect(validateSourceUrl("https://youtu.be/jNQXAC9IVRw").toString()).toBe(
      validateYouTubeUrl("https://youtu.be/jNQXAC9IVRw").toString(),
    );
  });

  it("explains that a Meet room URL is not a recording", () => {
    expect(() =>
      validateSourceUrl("https://meet.google.com/abc-defg-hij"),
