...
describe("YouTube resolver", () => {
  it("accepts common video URLs and canonicalizes them", () => {
    ...
    expect(
      youtubeVideoId(`https://www.youtube.com/live/${id}?feature=share`),
    ).toBe(id);
    expect(youtubeVideoId(`https://www.youtube-nocookie.com/embed/${id}`)).toBe(
      id,
    );
    expect(
      validateYouTubeUrl(
        `https://m.youtube.com/watch?v=${id}&list=ignored`,
      ).toString(),
  ...
  it("rejects playlists, channels, malformed IDs, and lookalike hosts", () => {
    expect(() =>
      validateYouTubeUrl("https://www.youtube.com/playlist?list=PL123"),
    ).toThrow("één video");
    expect(() =>
      validateYouTubeUrl("https://www.youtube.com/@example"),
    ).toThrow("één video");
    expect(() =>
      validateYouTubeUrl("https://www.youtube.com/watch?v=short"),
    ).toThrow("één video");
    expect(() =>
      validateYouTubeUrl("https://youtube.com.example/watch?v=jNQXAC9IVRw"),
    ).toThrow("YouTube");
  });
  ...
  it("maps public metadata to a generic episode", () => {
    ...
    expect(
      youtubeEpisodeFromMetadata(sourceUrl, {
        title: "Me at the zoo",
        channel: "jawed",
  ...
  it("allows an unlisted recording and ignores malformed dates and durations", () => {
    const episode = youtubeEpisodeFromMetadata(
      "https://www.youtube.com/watch?v=jNQXAC9IVRw",
      {
