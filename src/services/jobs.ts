...
const pendingRuns: Array<{ username: string; job: Job; type: "full" | "article" }> = [];
const pendingJobIds = new Set<string>();
let shuttingDown = false;

function userDirectory(username: string): string {
  if (!/^[a-z][a-z0-9_-]{1,31}$/.test(username)) throw new Error("Ongeldige gebruikersnaam.");
...
export function playbackFileForJob(username: string, id: string): string | undefined {
  return /^[0-9a-f-]{36}$/i.test(id) ? path.join(userDirectory(username), "media", `${id}.mp3`) : undefined;
}
...
async function update(username: string, job: Job, patch: Partial<Job>): Promise<void> {
  Object.assign(job, patch);
  await persist(username, job);
}

export async function createJob(username: string, input: Pick<Job, "sourceUrl" | "language" | "articleLength">): Promise<Job> {
...
export async function getJob(username: string, id: string): Promise<Job | undefined> {
...
export async function setArticleRead(username: string, id: string, read: boolean): Promise<ArticleSummary> {
...
export async function retryArticle(username: string, id: string): Promise<Job> {
  ...
  const job = await getJob(username, id);
  if (!job) throw new Error("Opdracht niet gevonden.");
  if (activeRuns.has(key) || pendingJobIds.has(key)) throw new Error("Deze opdracht wordt al verwerkt.");
...
function enqueueJob(username: string, job: Job, type: "full" | "article"): void {
  const key = jobKey(username, job.id);
  if (shuttingDown || activeRuns.has(key) || pendingJobIds.has(key)) return;
...
function drainQueue(): void {
  ...
  if (next.type === "article") launchArticleRetry(next.username, next.job);
  else launchJob(next.username, next.job);
}
...
function launchJob(username: string, job: Job): void {
  const key = jobKey(username, job.id);
  if (shuttingDown || activeRuns.size > 0 || activeRuns.has(key)) return;
...
function launchArticleRetry(username: string, job: Job): void {
  const key = jobKey(username, job.id);
  if (shuttingDown || activeRuns.size > 0 || activeRuns.has(key) || !job.transcript || !job.episode) return;
  const controller = new AbortController();
...
async function processArticleRetry(username: string, job: Job, signal: AbortSignal): Promise<void> {
  try {
    const article = await generateArticle(username, job, job.transcript!, job.episode!, signal);
    await update(username, job, { article, stage: "complete", progress: 100, message: "Artikel en transcript zijn klaar", completedAt: new Date().toISOString() });
    jobLog(job.id, "complete", "Artikel-only retry afgerond", { articleTitle: article.title });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Onbekende fout";
    if (signal.aborted) {
      await update(username, job, { stage: "queued", progress: 82, message: "Server afgesloten; artikelretry kan worden hervat", error: undefined });
    } else {
      jobError(job.id, job.stage, error);
      await update(username, job, { stage: "failed", error: message, message, progress: job.progress });
    }
  }
}
...
async function processJob(username: string, job: Job, signal: AbortSignal): Promise<void> {
  ...
  try {
    ...
    await update(username, job, { stage: "resolving", progress: 8, message: "Openbare opnamebron controleren" });
    ...
    await update(username, job, { episode, progress: 18, message: "Opname gevonden" });

    const input = path.join(workspace, "source.media");
    const downloadStartedAt = Date.now();
    jobLog(job.id, "downloading", "Media downloaden");
    await update(username, job, { stage: "downloading", progress: 22, message: "Opname veilig downloaden" });
    ...
    await update(username, job, { progress: 28, message: "Audio uit opname halen" });
    const playbackAudio = path.join(workspace, "playback.mp3");
    await normalizeAudio(input, playbackAudio, signal);
    await update(username, job, { progress: 32, message: "Audio opdelen voor transcriptie" });
    ...
    await update(username, job, { stage: "transcribing", progress: 36, message: `Transcriptie starten (${chunks.length} ${chunks.length === 1 ? "deel" : "delen"})` });
    const transcript = await transcribeChunks(chunks, job.language, (done, total) => {
      void update(username, job, { progress: 36 + Math.round((done / total) * 40), message: `Transcriptie ${done}/${total}` });
    }, (message, data) => {
      ...
      if (message.startsWith("Nog in afwachting")) {
        ...
        void update(username, job, { message: `${data.chunk}: wacht ${Math.max(1, Math.round(waitingSeconds / 60))} min. op OpenAI` });
      }
    }, signal);
    ...
    await update(username, job, { transcript, progress: 78, message: "Transcript compleet" });

    const article = await generateArticle(username, job, transcript, episode, signal);
    await update(username, job, { article, stage: "complete", progress: 100, message: "Artikel en transcript zijn klaar", completedAt: new Date().toISOString() });
    ...
    if (signal.aborted) {
      ...
      await update(username, job, { stage: "queued", progress: 2, message: "Server afgesloten; opdracht wordt na herstart hervat", error: undefined });
    } else {
      jobError(job.id, job.stage, error);
      await update(username, job, { stage: "failed", error: message, message, progress: job.progress });
    }
...
async function generateArticle(username: string, job: Job, transcript: NonNullable<Job["transcript"]>, episode: NonNullable<Job["episode"]>, signal: AbortSignal) {
  await update(username, job, { stage: "writing", progress: 82, message: "Brongebonden blogartikel schrijven" });
  ...
  return writeArticle(transcript, {
    ...
    if (message.startsWith("Nog in afwachting")) void update(username, job, { message: `Artikel wordt geschreven · ${Math.max(1, Math.round(Number(data.waitingSeconds ?? 0) / 60))} min. wachten` });
  }, signal);
}
