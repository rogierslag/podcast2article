...
export function audioChunkSeconds(): number {
  const configured = Number(
    process.env.AUDIO_CHUNK_SECONDS ?? DEFAULT_CHUNK_SECONDS,
  );
...
export async function downloadMedia(
  ...
  const configuredTimeout =
    options.timeoutMs ??
    Number(process.env.MEDIA_DOWNLOAD_TIMEOUT_MS ?? 900_000);
  const timeoutMs =
  ...
  const response = await safeFetch(url, {
    headers: { "User-Agent": "Podcast2Article/0.1 (+open-source)" },
  ...
  if (!response.ok || !response.body) {
    const permissionHint =
      response.status === 401 || response.status === 403
        ...
        : "";
    throw new Error(
      `Media downloaden mislukt (${response.status}).${permissionHint}`,
    );
  }
  const contentType = (
...
function runFfmpeg(args: string[], signal?: AbortSignal): Promise<void> {
  if (!ffmpegPath) {
    throw new Error(
      "De meegeleverde FFmpeg-binary is niet beschikbaar voor dit platform.",
    );
  }
  return new Promise((resolve, reject) => {
    ...
    process.once("close", (code) => {
      ...
      return code === 0
        ...
        : reject(
            new Error(
              `Audio verwerken mislukt: ${stderr.split("\n").at(-2) ?? `FFmpeg-code ${code}`}`,
            ),
          );
    });
  });
...
export function audioSplitArguments(
  input: string,
