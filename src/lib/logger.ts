...
export function jobLog(
  ...
  console.log(`${new Date().toISOString()} INFO  [job:${jobId}] [${stage}] ${clean(message)}${details(data)}`);
}

export function jobError(jobId: string, stage: string, error: unknown): void {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`${new Date().toISOString()} ERROR [job:${jobId}] [${stage}] ${clean(message)}`);
  if (error instanceof Error && error.stack && process.env.LOG_STACKS === "true") {
