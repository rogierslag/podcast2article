...
Keys are stable: `<prefix>/v1/users/<username>/<job-id>.json`.
...
Failures are logged and retried every 60 seconds while the server runs, or on its next startup.
Uploads run sequentially, with up to three SDK attempts within a 30-second request deadline.
...
Use a separate operator credential with `s3:GetObject` for restore, and `s3:ListBucket` only if inventory is needed.
...
Rerun to recover failed uploads.
...
For an actual erasure request, remove the local job and its receipt with the server stopped, then have a privileged operator remove **all versions** of its S3 key.
...
The command fetches the current object, validates schema version, owner and job ID, then atomically creates `data/users/owner/jobs/<job-id>.json` with mode `0600`.
...
Planning assumptions: AWS S3 Standard in Ireland (`eu-west-1`), one 100 KiB JSON object per article, one PUT per new article, no reads during normal operation, and no chargeable noncurrent versions.
Article sizes and production volume have not been measured.
